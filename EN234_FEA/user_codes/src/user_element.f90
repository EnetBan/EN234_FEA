...
subroutine user_element_static(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    else if ( element_identifier == 1002 ) then              ! B-Bar fully integrated 3D linear elastic element
    ...
    else if ( element_identifier == 1003 ) then              ! B-Bar hypoelastic element
    ...
    else if ( element_identifier == 1004 ) then              ! hyperelastic element
    ...
    else if ( element_identifier == 1005 ) then              ! cahn hilliard diffusion element

        call el_cahnhill_2d(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties, element_coords, length_coord_array, &                      ! Input variables
    dof_increment, dof_total, length_dof_array, &                                                ! Input variables
    n_state_variables, initial_state_variables, &                                                ! Input variables
    updated_state_variables,element_stiffness,element_residual, fail)      ! Output variables

    else if ( element_identifier == 102 ) then           ! 2D Plane Strain
    ...
    else if ( element_identifier == 103) then           ! 2D B-bar element
    ...
    else if ( element_identifier == 104) then           ! crack tip element

        call el_cracktip_2d(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties, element_coords, length_coord_array, &                      ! Input variables
    dof_increment, dof_total, length_dof_array, &                                                ! Input variables
    n_state_variables, initial_state_variables, &                                                ! Input variables
    updated_state_variables,element_stiffness,element_residual, fail)      ! Output variables

    else if ( element_identifier == 957) then           ! crack tip element

        call el_timoshenko_3d(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties, element_coords, length_coord_array, &                      ! Input variables
    dof_increment, dof_total, length_dof_array, &                                                ! Input variables
    n_state_variables, initial_state_variables, &                                                ! Input variables
    updated_state_variables,element_stiffness,element_residual, fail)      ! Output variables
...
subroutine user_element_dynamic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    if ( element_identifier == 1001 ) then              ! Basic fully integrated 3D linear elastic element
        ...
        call el_dynfrac_dynamic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
            n_properties, element_properties,element_coords, length_coord_array, &                         ! Input variables
            dof_increment, dof_total, length_dof_array,  &                                                 ! Input variables
            n_state_variables, initial_state_variables, &                                                  ! Input variables
            updated_state_variables,element_residual,element_deleted)                                      ! Output variables
...
subroutine user_element_fieldvariables(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    real( prec ), intent( out )   :: nodal_fieldvariables(n_field_variables,n_nodes)        ! Element stiffness (ROW,COLUMN)
        ...
        else if ( element_identifier == 1003 ) then              ! hypoelastic element
        ...
        else if ( element_identifier == 1004 ) then              ! hypoelastic element
        call fieldvars_hyperelast_3d(lmn, element_identifier, n_nodes, node_property_list, &         ! Input variables
        ...
        else if ( element_identifier == 103 ) then
            call fieldvars_linelast_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
                n_properties, element_properties,element_coords, length_coord_array, &                                   ! Input variables
                dof_increment, dof_total, length_dof_array, &                                                            ! Input variables
                n_state_variables, initial_state_variables,updated_state_variables, &                                    ! Input variables
        ...
        else if ( element_identifier == 104 ) then
            call fieldvars_cracktip_2d(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
                n_properties, element_properties,element_coords, length_coord_array, &                                   ! Input variables
                dof_increment, dof_total, length_dof_array, &                                                            ! Input variables
                n_state_variables, initial_state_variables,updated_state_variables, &                                    ! Input variables
                n_field_variables,field_variable_names, &                                                                ! Field variable definition
            ...
            call fieldvars_dynfrac(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
                n_properties, element_properties,element_coords, length_coord_array, &                                   ! Input variables
                dof_increment, dof_total, length_dof_array, &                                                            ! Input variables
                n_state_variables, initial_state_variables,updated_state_variables, &                                    ! Input variables
                n_field_variables,field_variable_names, &                                                                ! Field variable definition
